...
public final class TFM_SeniorWorld extends TFM_CustomWorld
{
    ...
    private static final String WORLD_NAME = "seniorworld";
    //
    ...
    protected World generateWorld()
    {
        ...
        welcomeSign.setLine(0, ChatColor.GREEN + "SeniorWorld");
    ...
    {
        if (supervisor == guest || TFM_AdminList.isSuperAdmin(supervisor))
        {
            return false;
        }

        if (TFM_AdminList.isSeniorAdmin(supervisor))
        {
        ...
        if (world != null && event.getTo().getWorld() == world)
        {
            ...
            if (!canAccessWorld(player))
            {
                ...
                if (lastTP == null || lastTP.longValue() + TP_COOLDOWN_TIME <= currentTimeMillis)
                {
                    ...
                    TFM_Log.info(player.getName() + " attempted to access the SeniorWorld.");
                    new BukkitRunnable()
                    {
                        @Override
                        public void run()
    ...
    public boolean canAccessWorld(final Player player)
    {
        ...
        if (cached == null)
        {
            boolean canAccess = TFM_AdminList.isSeniorAdmin(player);
            if (!canAccess)
            {
                Player supervisor = guestList.get(player);
                canAccess = supervisor != null && supervisor.isOnline() && TFM_AdminList.isSeniorAdmin(supervisor);
                if (!canAccess)
                {
    ...
    public static TFM_SeniorWorld getInstance()
    {
        return TFM_SeniorWorldHolder.INSTANCE;
    }

    private static class TFM_SeniorWorldHolder
    {
        private static final TFM_SeniorWorld INSTANCE = new TFM_SeniorWorld();
    }
}
